...
const CourseComponent = ({ e, setdelete_id, setdelete_open }) => {
    ...
    return (
        authContext?.user?.user_details?.is_teacher ? (
            <div key={e?.id} className='course border p-2 px-4 rounded-md text-end
                            flex flex-row-reverse justify-between
                         '>
                <div className='flex flex-row-reverse gap-2'>
                    <div className='my-auto'>
                        <Image alt='' className='max-w-[100px] max-h-[70px]' width={100} height={100} src={server + e?.thumbnail} />
                    </div>
                ...
                <div className='options flex flex-col gap-1 my-auto text-start'>
                    <Link href={`/course/${e?.id}`}>
                        <button className='p-0 px-3 bg-sky-300 hover:bg-black'>edit</button>
                    </Link>
                    <button onClick={() => {
                        setdelete_id(e?.id)
                        setdelete_open(true)
                    }} className='p-0 px-3 bg-red-300 hover:bg-black'>delete</button>
                </div>
            </div>
        ) :
            <>
                {
                    path == '/saved' ? (
                        <div className='flex flex-row-reverse gap-3 flex-wrap'>
                            ...
                            <Link href={`/course/${e?.id}`} className='flex flex-row-reverse gap-3 flex-wrap bg-white p-3 w-full shadow-md transition-all hover:px-5 hover:rounded-3xl'>
                                <div>
                                ...
                                <div className='text-end my-auto'>
                                    <strong>{e?.title}</strong>
                                    <p>{e?.description?.length > 50 ? "..."+e?.description?.slice(0, 50) : e?.description}</p>
                        ...
                        <>
                            ...
                            <div className='course border p-2 px-4 rounded-md text-end
